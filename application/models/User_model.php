...
class User_model extends CI_Model {
    ...
    public function get_class_enrolled_students($class_id){
        if ($class_id > 0 && $class_id !='') {
            return $this->db->query('SELECT USR.first_name , USR.last_name, USR.email , USR.id , ENRL.user_id , ENRL.course_id, ENRL.id EnrollId from users USR INNER JOIN enrol ENRL on ENRL.user_id = USR.id WHERE ENRL.class_id = ' . $class_id );
        }
    }

    public function delete_enrolled_user($enrollId)
    {
        $this->db->query("DELETE from enrol where id = " . $enrollId);
    }
    ...
    public function add_user($role_id = 2, $class_id = '') {
        ...
        if ($validity == false) {
            ...
            if ($data['class_id'] != '' && $data['class_id'] > 0) {
                ...
                $this->crud_model->enrol_a_student($course_id, $user_id , $data['class_id']);
            }
    ...
    public function get_course($classId)
    {
        $cls = $this->db->get_where('classes', array('id' => $classId))->row_array();
