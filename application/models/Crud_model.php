...
class Crud_model extends CI_Model
{
    ...
    public function get_institute_live_sessions()
    {
        $instructor_ids = array();
        ...
        $course_ids = $this->db->get('course')->result_array();
        foreach ($course_ids as $course) {
            array_push($course_array_ids, $course['id']);
        }
        
        $this->db->where_in('course_id', $course_array_ids);
        $live_sessions = $this->db->get('live_sessions')->result_array();
       

        foreach($live_sessions as $row)
        {
            ...
            if ($institute_id > 0){
                ...
                if (sizeof($course_ids)) {
                 ...
                 if (count($course_classes) >= $plan['classes']){
                    $this->session->set_flashdata('error_message', get_phrase('you_increase_class_limit'));
                 }else{
    ...
    {
        return $this->db->query("SELECT U.first_name , U.last_name, U.id, U.email from users U where U.id NOT IN ( SELECT ENRL.user_id from users USR INNER JOIN enrol ENRL on ENRL.user_id = USR.id WHERE ENRL.course_id = ".$courseid." ) AND U.role_id = 2")->result();
    }
    ...
    public function get_institute_by_course($id = "")
    {
    ...
    public function delete_section($course_id, $section_id)
    {
        ...
        $s3_setting = $s3_model->create_s3_object($s3_setting);

        $lessons  = $this->db->query("SELECT * FROM lesson where section_id  = " . $section_id)->result_array();
        ...
        {
            if($row['video_type'] == "amazon")
            {
                $s3_model->deleteFile($s3_setting , str_replace('https://clms-storage.s3.eu-west-1.amazonaws.com/','',$row['video_url']));
            }
        }
    }
        ...
        if ($lesson_type == 'video') {
            ...
            elseif($lesson_provider == 's3'){
                ...
                if ($space_validity == false){
                    ...
                    $s3_model = new S3_model();
                    $s3_setting = $s3_model->create_s3_object($s3_setting);
                    $s3_model->deleteFile($s3_setting , $this->input->post('video_file_for_amazon_s3'));
                }else
                {
                    $video_size = $this->input->post('video_file_for_amazon_s3_size');
                    ...
                    if ($cloud_space >= $video_kb_size){
                            ...
                            $fileName = $this->input->post('video_file_for_amazon_s3');
                            ...
                            $data['video_url'] = 'https://clms-storage.s3.eu-west-1.amazonaws.com/'.$this->input->post('video_file_for_amazon_s3');
                            ...
                            $data['video_url_for_mobile_application'] = 'https://clms-storage.s3.eu-west-1.amazonaws.com/'.$this->input->post('video_file_for_amazon_s3');
    ...
    public function edit_lesson($lesson_id,  $institute_name = '')
    {
        ...
        if($this->input->post('video_file_for_amazon_s3') != "" || $this->input->post('video_file_for_amazon_s3') != NULL)
        {
            $s3_model->deleteFile($s3_setting , str_replace('https://clms-storage.s3.eu-west-1.amazonaws.com/','',$previous_data['video_url']));
        }
        ...
        if ($lesson_type == 'video') {
            ...
            if ($lesson_provider == 'youtube' || $lesson_provider == 'vimeo') {
               ...
               if($this->input->post('video_file_for_amazon_s3') != '')
               {
                    $space_validity = $this->check_institute_membory_limit($course['institute_id']);
                if ($space_validity == false){
                    $this->session->set_flashdata('error_message', get_phrase('You do not have more storage'));
                ...
                $data['video_url'] = 'https://clms-storage.s3.eu-west-1.amazonaws.com/'.$this->input->post('video_file_for_amazon_s3');
                $data['video_type'] = 'amazon';
                $data['video_size'] = $this->input->post('video_file_for_amazon_s3_size');
                $data['lesson_type'] = 'video';
                $data['attachment_type'] = 'url';
    ...
    public function delete_lesson($lesson_id)
    {
        ...
        if($lesson['video_type'] == "amazon")
        {
            $s3_setting = $this->db->get('s3_settings')->row_array();
            $s3_model = new S3_model();
            $s3_setting = $s3_model->create_s3_object($s3_setting);
            $s3_model->deleteFile($s3_setting , str_replace('https://clms-storage.s3.eu-west-1.amazonaws.com/','',$lesson['video_url']));
        }
        $this->db->where('id', $lesson_id);
    ...
    public function enrol_a_student($course_id = '', $user_id = '', $class_id = '')
    {
        $data['course_id'] = $course_id;
        $data['user_id'] = $user_id;
    ...
    public function send_new_private_message()
    {
        ...
        if ($receiver_type == 'class'){
            ...
            foreach ($receivers as $std) {
                ...
                redirect(site_url($role.'/message/message_read/' . $message_thread_code), 'refresh');
            }
            }else{
                $this->session->set_flashdata('flash_message', get_phrase('class_has_not_students,Please_create_a_students'));
                redirect(site_url($role.'/message/message_new/'));
            }
            ...
            if (count($receiver) > 0) {  
                ...
                $this->session->set_flashdata('flash_message', get_phrase('Student_not_found_Please_create_a_students'));
                redirect(site_url($role.'/message/message_new/'));
            }
        }
